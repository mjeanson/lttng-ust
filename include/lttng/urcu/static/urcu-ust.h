...
struct lttng_ust_urcu_reader {
	...
	struct lttng_ust_list_head node __attribute__((aligned(LTTNG_UST_CACHE_LINE_SIZE)));
	pthread_t tid;
...
static inline void _lttng_ust_urcu_read_lock(void)
{
	...
	lttng_ust_urcu_assert((tmp & LTTNG_UST_URCU_GP_CTR_NEST_MASK) != LTTNG_UST_URCU_GP_CTR_NEST_MASK);
