...
ino_t get_uts_ns(void)
{
	...
	uts_ns = LTTNG_UST_LOAD_SHARED(URCU_TLS(cached_uts_ns));

	/*
	 * If the cache is populated, do nothing and return the
	 * cached inode number.
	 */
	if (lttng_ust_likely(uts_ns != NS_INO_UNINITIALIZED))
		return uts_ns;
	 ...
	 */
	LTTNG_UST_STORE_SHARED(URCU_TLS(cached_uts_ns), uts_ns);
...
{
	LTTNG_UST_STORE_SHARED(URCU_TLS(cached_uts_ns), NS_INO_UNINITIALIZED);
}
