...
gid_t get_vgid(void)
{
	...
	vgid = LTTNG_UST_LOAD_SHARED(cached_vgid);

	if (lttng_ust_unlikely(cached_vgid == (gid_t) -1)) {
		vgid = getgid();
		LTTNG_UST_STORE_SHARED(cached_vgid, vgid);
	}
...
{
	LTTNG_UST_STORE_SHARED(cached_vgid, INVALID_GID);
}
