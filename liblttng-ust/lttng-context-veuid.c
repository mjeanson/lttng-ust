...
uid_t get_veuid(void)
{
	...
	veuid = LTTNG_UST_LOAD_SHARED(cached_veuid);

	if (lttng_ust_unlikely(veuid == INVALID_UID)) {
		veuid = geteuid();
		LTTNG_UST_STORE_SHARED(cached_veuid, veuid);
	}
...
{
	LTTNG_UST_STORE_SHARED(cached_veuid, INVALID_UID);
}
